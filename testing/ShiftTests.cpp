...
TEST_F(ShiftTests, LSLCases)
{
    ...
    {
        {0x00000001,   1, 0x00000002, 0, 0},       // simple shift left
        {0x80000000,   1, 0x00000000, 1, 0},       // MSB shifts out → carry=1
        {0x12345678,   4, 0x23456780, 1, 0},       // mid shift
        {0xFCFA0101,  32, 0x00000000, 1, 0},       // shift == 32
        {0xFCFA0101,  33, 0x00000000, 0, 0},       // shift > 32
        {0x000000FF,   0, 0x000000FF, 1, 0x20000000} // shift==0, reg form, preserves carry-in=1
    };
...
TEST_F(ShiftTests, LSRCases)
{
    ...
    {
        RunCase(tc, cpu, &ShiftOp2_LSR_Immediate);
    }
}
...
TEST_F(ShiftTests, ASRCases)
{
    ...
    {
        {0x80000000,   1, 0xC0000000, 0, 0},       // sign extend negative
        {0x70000000,   4, 0x07000000, 0, 0},       // positive value
        {0xF0000000,   4, 0xFF000000, 0, 0},       // negative value
        {0x12345678,   0, 0x00000000, 0, 0},       // imm shift=0 → ASR #32
        {0x80000000,   0, 0xFFFFFFFF, 1, 0},       // imm shift=0 → ASR #32 all ones
        {0xAAAAAAAA,   0, 0xAAAAAAAA, 1, 0x20000000}, // reg shift=0 → preserves carry-in
        {0x80000000,  40, 0xFFFFFFFF, 1, 0},       // shift > 32 → all sign bits
        {0x70000000,  40, 0x00000000, 0, 0}        // shift > 32 → all sign bits
    };

    for (auto& tc : asrCases) 
    {
        RunCase(tc, cpu, &ShiftOp2_ASR_Immediate);
    }
}
...
TEST_F(ShiftTests, RORCases)
{
    ...
    {
        {0x80000001,   1, 0xC0000000, 1, 0},       // rotate right 1
        {0x12345678,   4, 0x81234567, 1, 0},       // rotate right 4
        {0xAAAAAAAA,   8, 0xAAAAAAAA, 1, 0},       // symmetric pattern
        {0xAAAAAAAA,   0, 0xD5555555, 0, 0x20000000} // shift=0 → triggers RRX
    };

    for (auto& tc : rorCases) 
    {
        RunCase(tc, cpu, &ShiftOp2_ROR_Immediate);
    }
}
...
void RunCase(const ShiftTestCase& testcase, TestCPU& cpu, std::function<ShifterOperand(uint32_t, uint32_t, GBA_CPU&)> shiftFunc)
{
    ...
    ShifterOperand result = shiftFunc(testcase.value, testcase.shift, cpu);
...
{
    cpu.SetCPSR(testcase.cpsr);
    ShifterOperand result = ShiftOp2_LSL_Immediate(testcase.value, testcase.shift, cpu);
    EXPECT_EQ(result.value, testcase.expectedValue);
    EXPECT_EQ(result.carryOut, testcase.expectedCarry);
}